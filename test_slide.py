...
if __name__ == '__main__':
    ...
    try:
        ...
        slidepath = rootpath + '/' + os.listdir(rootpath)[0]
        print(slidepath)
    except IndexError:
...
# 组合图片
def compose(composed_size,factor=0.5):
    print(composed_size)
        ...
        for x in range(composed_size[0]):
            if (x, y) in image_numDict:
                ...
                if os.path.split(image_numDict[(x, y)])[-1] in list0:
                    image = add_transparency(image,factor=factor)
...
def something(factor=0.5):
    composed_size = get_size(image_dir)
    get_coordDict()
    compose(composed_size,factor=factor)
    ...
    for root, dirs, files in os.walk(test_image_dir):
        for f in files:
            ...
            if os.path.splitext(full_f1)[-1] == '.jpeg':
                ...
                # if probability >= 0.65:
                '''when threshold up to 0.65,
                    the proportion of wrong prediction was reduced from 0.4656 to 41.11
                    None：0.4656,0.5667,0.1541
                    0.55：0.4509
                    0.65：0.4111,/,0.2423
                    0.75：0.3921
                    0.85：0.3774,/,0.41
        ...
        sum0=0
        for i in dict:
            sum0=sum0+int(dict[i])
        for i in dict:
            print(i,int(dict[i])/sum0)
    time_end=time.time()
